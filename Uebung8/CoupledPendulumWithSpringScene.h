#pragma once
#include "SimulationScene.h"
#include "SimulationWindow.h"
#include "PhysicInterfaces/ParticleNode.h"

#include "R3D/ParticleEngine/Particle.h"
#include "R3D/ParticleEngine/ParticleGravity.h"
#include "R3D/ParticleEngine/ParticleCable.h"
#include "R3D/ParticleEngine/ParticleSpring.h"
#include "R3D/ParticleEngine/ParticleBidirectionalSpring.h"

#include "EC3D/Core/Node.h"

#include <string>
#include "../Cave.h"

class CoupledPendulumWithSpringScene : public SimulationScene
{
public:
	explicit CoupledPendulumWithSpringScene(const std::string& name, SimulationWindow* window);
	~CoupledPendulumWithSpringScene();

	void tick(float timeDelta) override;
	void reset() override;

private:
	Cave* m_cave;

	ec::Node* m_firstBallNode;
	ec::Node* m_secondBallNode;
	ec::Drawable* m_ballDrawable;

	r3::Particle* m_firstBallParticle;
	ParticleNode* m_firstBallParticleNode;

	r3::Particle* m_secondBallParticle;
	ParticleNode* m_secondBallParticleNode;

	r3::Particle* m_firstCableParticle;
	r3::Particle* m_secondCableParticle;

	r3::ParticleGravity* m_particleGravity;
	r3::ParticleCable* m_firstParticleCable;
	r3::ParticleCable* m_secondParticleCable;
	r3::ParticleBidirectionalSpring* m_particleSpring;
};
